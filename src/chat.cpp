...
std::string get_response(std::string input)
{
    ...
    if (!llama_model_load("C:/SoftwareProjects/Github/gpt4all.cpp/Release/gpt4all-lora-quantized.bin", model, vocab, 2048)) {
    ...
    if(!input.empty()) {
        std::vector<gpt_vocab::id> param_inp = ::llama_tokenize(vocab, input, true);
    ...
    llama_eval(model, std::min(4, (int32_t)std::thread::hardware_concurrency()), 0, { 0, 1, 2, 3 }, logits, mem_per_token);
    
    int last_n_size = 64;
    std::vector<gpt_vocab::id> last_n_tokens(last_n_size);
    std::fill(last_n_tokens.begin(), last_n_tokens.end(), 0);

    int remaining_tokens = 2048 - embd_inp.size();
    ...
    while (remaining_tokens > 0) {
        ...
        if (embd.size() > 0) {
            ...
            if (!llama_eval(model, std::min(4, (int32_t)std::thread::hardware_concurrency()), n_past, embd, logits, mem_per_token)) {
                ERR_PRINT("Failed to predict.");
                return "";
            }
        }
        ...
        if (embd_inp.size() <= input_consumed) {
            ...
            const float top_k = 40;
            const float top_p = 0.95f;
            const float temp  = 0.10f;
            const float repeat_penalty = 1.30f;
        ...
        for (auto id : embd) {
            accum += vocab.id_to_token[id].c_str();
        }
